        ...
        if ($id) {
            if ($password !== '') {
                if (strlen($password) < 8) {
                    flash_set('danger', 'Password must be at least 8 characters.');
                    redirect(BASE_URL . '/marinka/admin-user-form.php?id=' . $id);
                }
...
<form method="post" class="stat-card bg-white p-4">
  ...
  <a href="<?= e(BASE_URL) ?>/marinka/admin-users.php" class="btn btn-outline-secondary rounded-pill px-4">Cancel</a>
</form>

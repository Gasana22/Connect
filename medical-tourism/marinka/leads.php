...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    ...
    redirect(BASE_URL . '/marinka/leads.php');
}
...
<div class="d-flex justify-content-between align-items-center mb-3">
  ...
  <div class="btn-group">
    <a href="<?= e(BASE_URL) ?>/marinka/leads.php" class="btn btn-sm btn-outline-secondary <?= $statusFilter === '' ? 'active' : '' ?>">All</a>
    <a href="<?= e(BASE_URL) ?>/marinka/leads.php?status=new" class="btn btn-sm btn-outline-secondary <?= $statusFilter === 'new' ? 'active' : '' ?>">New</a>
    <a href="<?= e(BASE_URL) ?>/marinka/leads.php?status=contacted" class="btn btn-sm btn-outline-secondary <?= $statusFilter === 'contacted' ? 'active' : '' ?>">Contacted</a>
    <a href="<?= e(BASE_URL) ?>/marinka/leads.php?status=converted" class="btn btn-sm btn-outline-secondary <?= $statusFilter === 'converted' ? 'active' : '' ?>">Converted</a>
    <a href="<?= e(BASE_URL) ?>/marinka/leads.php?status=closed" class="btn btn-sm btn-outline-secondary <?= $statusFilter === 'closed' ? 'active' : '' ?>">Closed</a>
  </div>
</div>
    ...
    <table class="table align-middle">
      ...
      <?php foreach ($leads as $lead): ?>
        <tr>
          ...
          <td class="text-end">
            <a href="<?= e(BASE_URL) ?>/marinka/lead-view.php?id=<?= (int)$lead['id'] ?>" class="btn btn-sm btn-outline-primary"><i class="bi bi-eye"></i></a>
